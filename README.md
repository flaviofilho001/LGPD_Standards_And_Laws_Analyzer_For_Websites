...
[![GraphRAG](https://img.shields.io/badge/Engine-GraphRAG%20%2B%20NetworkX-purple.svg)](https://networkx.org/)
...
- 🧠 **Motor GraphRAG com Grafo de Conhecimento (NetworkX):** Expansão de vizinhança ($k$-hops) sobre triplas regulatórias para recuperar contexto exato antes do envio ao LLM.
...
- 🧠 **Gerador Automático de Triplas (`gerador_triplas.py`):** IA extrai triplas inéditas da base de conhecimento e converte arquivos ontológicos `.ttl` (Turtle / RDF / OWL) para o grafo.
- 🕸️ **Gerenciador de Triplas Persistente (`triplas.json`):** Deduplicação automática com chave única `(origem, relação, destino)`.
...
- 📊 **Interface Visual Interativa (Streamlit):** Dashboard web completo com KPIs, filtros por status, progresso em tempo real, visualização de subgrafos e exportação de relatórios.
- 📄 **Relatórios Multi-Formato:** Exportação automática em `relatorio_auditoria.json` e `relatorio_auditoria.md` (com alertas e badges do GitHub).
...
├── triplas.json              # Grafo de Conhecimento de triplas deduplicadas (42+ triplas)
├── compliance_graphrag.py    # Motor principal de auditoria GraphRAG (10 tópicos & paralelização)
├── gerador_triplas.py        # Gerador automático de triplas via IA e conversor de arquivos .ttl (RDF)
...
### 4. Executando via Linha de Comando (CLI)

```bash
python run_audit_pipeline.py https://www.vizinhub.com.br/privacy https://www.vizinhub.com.br/terms
```

### 5. Executando o Gerador Automático de Triplas (`gerador_triplas.py`)
...
Para converter um arquivo de ontologia RDF `.ttl` para o `triplas.json`:

```bash
python gerador_triplas.py --ttl caminho/para/minha_ontologia.ttl
```
